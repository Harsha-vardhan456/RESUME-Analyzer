...
class ResourceSearchReq(BaseModel):
    query: str
...
class AssignedCompanyTest(BaseModel):
    candidate: str
    companyName: str
    testType: str
    questions: List[Question]
...
# Configure Gemini AI
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY", "AIzaSyCEfBjtgmX3YCBoMUbnIacHXo6BbsIVMX0")
...
async def analyze_resume_with_ai(resume_text: str, job_description: str):
    ...
    try:
        ...
        prompt = f"""
        You are an expert HR analyst and technical recruiter. Analyze the following resume against the job description and provide a comprehensive assessment.
        ...
        {{
            "suitabilityScore": <number from 0-100>,
            "missingSkills": ["skill1", "skill2"],
            "capabilityAnalysis": "<detailed analysis including: strengths, weaknesses, experience match, technical skills assessment, and specific recommendations for improvement>",
        ...
        Analysis Guidelines:
        1. **Technical Skills**: Evaluate programming languages, frameworks, tools mentioned
        2. **Experience Relevance**: Assess if work experience aligns with job requirements
        3. **Project Analysis**: Review projects and their relevance to the role
        4. **Education Fit**: Consider educational background and certifications
        5. **Soft Skills**: Evaluate communication, leadership, teamwork indicators
        6. **Gap Analysis**: Identify missing skills or experience
        7. **Improvement Suggestions**: Provide specific, actionable recommendations
        ...
        try:
            ...
            # Find JSON in the response (handle markdown formatting)
            ...
            return {
                "suitabilityScore": analysis_data.get("suitabilityScore", 50),
                "missingSkills": analysis_data.get("missingSkills", []),
                "capabilityAnalysis": analysis_data.get("capabilityAnalysis", "Analysis completed successfully."),
            ...
            logger.warning("Failed to parse AI response as JSON, using fallback analysis")
            return {
                "suitabilityScore": 60,
                "missingSkills": ["Advanced skills may be required"],
                "capabilityAnalysis": "AI analysis completed. Review the resume manually for detailed assessment."
            }
...
async def get_ai_resource_recommendations(query: str, category: Optional[str] = None):
    """
    Get AI-powered resource recommendations using Google Gemini
    """
    try:
        # Initialize Gemini model
        model = genai.GenerativeModel('gemini-2.0-flash-exp')
        ...
        prompt = f"""
        Provide learning resources for: "{query}"{category_filter}
        ...
        {{
            "resources": [
                {{
                    ...
                    "type": "Video/Course/Documentation/Book/Article"
                }}
            ],
            "summary": "Brief summary of recommended learning path"
        }}
        
        Focus on:
        1. High-quality, reputable learning resources
        2. Mix of free and paid resources
        3. Different difficulty levels
        4. Various learning formats (videos, courses, documentation, books)
        5. Include popular platforms like Coursera, Udemy, freeCodeCamp, MDN, etc.
        ...
        try:
            ...
            return {
                ...
                "summary": recommendations.get("summary", "AI-powered recommendations generated successfully.")
            }
            
        except json.JSONDecodeError:
            # Fallback to curated resources if JSON parsing fails
            logger.warning("Failed to parse AI response as JSON, using fallback resources")
            return {
                "success": True,
                "resources": [
                    {
                ...
                "summary": "Here are some excellent learning resources to get you started."
            }
            
    except Exception as e:
        logger.error(f"Error in AI resource recommendations: {e}")
        ...
        return {
            ...
            "summary": "Basic learning resources to help you get started."
        }
...
async def delete_job(job_id: str, user=Depends(recruiter_required)):
    try:
...
async def update_application(app_id: str, application_data: dict, user=Depends(recruiter_required)):
    try:
        ...
        logger.info(f"Updating application {app_id} with data: {application_data}")
        ...
        logger.info(f"Application updated successfully: {result}")
...
async def submit_application(
    ...
    try:
        ...
        await db.activities.insert_one({
            "description": f"Application submitted by {user['username']} for {job}",
            "timestamp": datetime.utcnow()
        })
        
        logger.info(f"Application submitted by {user['username']} for {job}")
...
async def candidate_analyze_resume(
    ...
    try:
        ...
        if not resume_text.strip():
            raise HTTPException(status_code=400, detail="Could not extract text from PDF. Please ensure the file is not corrupted.")
        
        # Analyze with AI
        analysis_result = await analyze_resume_with_ai(resume_text, jobDescription)
        ...
        analysis_doc = {
            ...
            "suitabilityScore": analysis_result.get("suitabilityScore", 0),
            "missingSkills": analysis_result.get("missingSkills", []),
            "capabilityAnalysis": analysis_result.get("capabilityAnalysis", ""),
        ...
        # Log activity
        await db.activities.insert_one({
            "description": f"Resume analyzed by {user['username']} for role: {jobRole}",
            "timestamp": datetime.utcnow()
        })
        
        logger.info(f"Resume analyzed by {user['username']} for role: {jobRole}")
...
@app.post("/api/candidate/resources")
async def get_resource_recommendations(req: ResourceSearchReq, user=Depends(candidate_required)):
    try:
        # Get AI-powered recommendations
        recommendations = await get_ai_resource_recommendations(req.query, req.category)
        
        # Log activity
        await db.activities.insert_one({
            "description": f"Resource search by {user['username']} for: {req.query}",
            "timestamp": datetime.utcnow()
        })
        
        logger.info(f"Resource recommendations generated for {user['username']} for query: {req.query}")
        
        return recommendations
        
    except Exception as e:
        logger.error(f"Error getting resource recommendations: {e}")
        raise HTTPException(status_code=500, detail="Failed to get resource recommendations")
...
@app.post("/api/recruiter/screen-application")
async def screen_application(
    request: ScreeningRequest,
    user=Depends(recruiter_required)
):
    try:
        # Create screening prompt for Gemini AI
        prompt = f"""
        You are an expert HR recruiter and technical evaluator using the ACHARYA Intelligent Screening Algorithm (AISA). Analyze the following candidate's resume against the job requirements and provide a comprehensive assessment using advanced AI algorithms.
        
        JOB TITLE: {request.jobTitle}
        JOB DESCRIPTION: {request.jobDescription}
        
        CANDIDATE RESUME: {request.resumeText}
        
        Please use the ACHARYA Intelligent Screening Algorithm (AISA) to evaluate the candidate:
        
        AISA EVALUATION CRITERIA:
        1. TECHNICAL SKILLS MATCH (0-1 point)
           - Analyze programming languages, frameworks, tools mentioned
           - Compare with job requirements
           - Score: 0.5 for basic match, 1.0 for strong match
        
        2. EXPERIENCE RELEVANCE (0-1 point)
           - Evaluate work experience alignment with job role
           - Consider industry experience and project complexity
           - Score: 0.5 for some relevance, 1.0 for high relevance
        
        3. EDUCATION FIT (0-1 point)
           - Assess educational background and certifications
           - Consider degree relevance and academic performance
           - Score: 0.5 for basic fit, 1.0 for excellent fit
        
        4. PROJECT PORTFOLIO (0-1 point)
           - Evaluate project complexity and relevance
           - Consider technical depth and innovation
           - Score: 0.5 for basic projects, 1.0 for impressive portfolio
        
        5. OVERALL SUITABILITY (0-1 point)
           - Holistic assessment of candidate potential
           - Consider soft skills, adaptability, growth potential
           - Score: 0.5 for potential, 1.0 for excellent fit
        
        AISA ALGORITHM FEATURES:
        - Pattern recognition for skill matching
        - Predictive analysis for role fit
        ...
        {{
            "score": <total_score_out_of_5>,
            "analysis": "<detailed_analysis_explaining_the_score_without_mentioning_specific_names>",
            "decision": "<ACCEPTED_or_REJECTED_based_on_score_4_or_above>",
            "strengths": ["strength1", "strength2"],
            "weaknesses": ["weakness1", "weakness2"],
            "recommendations": ["recommendation1", "recommendation2"],
            "technicalScore": <0-1>,
        ...
        AISA Scoring Guidelines:
        - Score 4-5: ACCEPTED for further consideration (High potential)
        - Score 1-3: REJECTED - does not meet minimum requirements
        
        Important: Do not mention any specific names in the analysis. Use generic terms like "the candidate", "this applicant", or "the individual".
        
        Be thorough, objective, and constructive in your analysis using the ACHARYA Intelligent Screening Algorithm (AISA).
        """
        ...
        try:
            ...
            score = min(max(analysis_data.get("score", 3), 1), 5)
            decision = "ACCEPTED" if score >= 4 else "REJECTED"
            
            result = {
                "success": True,
                "score": score,
                "analysis": analysis_data.get("analysis", "Analysis completed successfully."),
                "decision": decision,
                "strengths": analysis_data.get("strengths", []),
                "weaknesses": analysis_data.get("weaknesses", []),
                "recommendations": analysis_data.get("recommendations", []),
                "technicalScore": analysis_data.get("technicalScore", 0),
                "experienceScore": analysis_data.get("experienceScore", 0),
                "educationScore": analysis_data.get("educationScore", 0),
                "portfolioScore": analysis_data.get("portfolioScore", 0),
                "suitabilityScore": analysis_data.get("suitabilityScore", 0)
            }
            
            # Log the screening activity
            await db.activities.insert_one({
                "description": f"Application screened by {user['username']} for {request.candidate} - Score: {score}/5 - Decision: {decision}",
                "timestamp": datetime.utcnow()
            })
            
            logger.info(f"Application screened by {user['username']} for {request.candidate} - Score: {score}/5")
            ...
            return {
                "success": True,
                "score": 3,
                "analysis": "AI analysis completed. Manual review recommended for detailed assessment.",
                "decision": "ACCEPTED",
                "strengths": ["Basic qualification met"],
                "weaknesses": ["Detailed analysis unavailable"],
                "recommendations": ["Manual review recommended"]
            }
            
    except Exception as e:
        logger.error(f"Error in application screening: {e}")
        raise HTTPException(status_code=500, detail="Failed to screen application")

# ---------- COMPANY TEST APIs ----------
@app.post("/api/company-tests")
async def submit_company_test(test: CompanyTest, user=Depends(candidate_required)):
    try:
        test_doc = test.dict()
        test_doc["user"] = user["username"]
        
        await db.companytests.insert_one(test_doc)
        
        # Log activity
        await db.activities.insert_one({
            "description": f"Company test submitted by {user['username']}",
            "timestamp": datetime.utcnow()
        })
        
        return {"success": True, "message": "Company test submitted successfully"}
    except Exception as e:
        logger.error(f"Error submitting company test: {e}")
        raise HTTPException(status_code=500, detail="Failed to submit company test")

@app.post("/api/recruiter/assign-company-test")
async def assign_company_test(test_data: dict, user=Depends(recruiter_required)):
    try:
        test_doc = {
            "candidate": test_data["candidate"],
        ...
        result = await db.assignedcompanytests.insert_one(test_doc)
        test_id = str(result.inserted_id)
        
        logger.info(f"Company test assigned successfully. Test ID: {test_id}, Original ObjectId: {result.inserted_id}")
        
        # Log activity
        await db.activities.insert_one({
            "description": f"Company test assigned to {test_data['candidate']} by {user['username']}",
            "timestamp": datetime.utcnow()
        })
        
        return {"success": True, "message": "Company test assigned successfully", "test_id": test_id}
    except Exception as e:
        logger.error(f"Error assigning company test: {e}")
        raise HTTPException(status_code=500, detail="Failed to assign company test")

@app.post("/api/candidate/submit-company-test/{test_id}")
async def submit_company_test_answers(test_id: str, answers_data: dict, user=Depends(candidate_required)):
    try:
        # Find the company test
        company_test = await db.assignedcompanytests.find_one({"_id": ObjectId(test_id)})
        if not company_test:
            raise HTTPException(status_code=404, detail="Company test not found")
        
        if company_test["candidate"] != user["username"]:
            raise HTTPException(status_code=403, detail="Not authorized to submit this test")
        
        if company_test["status"] == "Completed":
            raise HTTPException(status_code=400, detail="Test already completed")
        
        # Evaluate the test
        eval_result = await evaluate_mock_test_with_ai(
            company_test["testType"],
            company_test["questions"],
            answers_data["answers"]
        )
        
        # Update the test with results
        await db.assignedcompanytests.update_one(
            {"_id": ObjectId(test_id)},
            {
                "$set": {
                    "status": "Completed",
        ...
        return {
            "success": True,
            "message": "Company test submitted successfully",
            "score": eval_result["score"],
            "evaluation": eval_result["evaluation"]
        }
    except Exception as e:
...
@app.get("/api/candidate/company-tests")
async def get_candidate_company_tests(user=Depends(candidate_required)):
    try:
        tests = await db.assignedcompanytests.find({
            "candidate": user["username"],
            "status": "Pending"
        }).to_list(length=100)
        ...
        logger.error(f"Error fetching company tests: {e}")
        raise HTTPException(status_code=500, detail="Failed to fetch company tests")

@app.get("/api/candidate/company-tests/{test_id}")
async def get_company_test_details(test_id: str, user=Depends(candidate_required)):
    try:
        logger.info(f"Fetching company test details for test_id: '{test_id}', user: {user['username']}")
        logger.info(f"Test ID type: {type(test_id)}, length: {len(test_id)}")
        ...
        try:
            ...
            logger.error(f"Invalid ObjectId format: '{test_id}', error: {e}")
            logger.error(f"ObjectId should be 24 characters, got: {len(test_id)}")
            raise HTTPException(status_code=400, detail=f"Invalid test ID format: {test_id}")
        
        test = await db.assignedcompanytests.find_one({"_id": object_id})
        if not test:
            logger.error(f"Company test not found for test_id: {test_id}")
            raise HTTPException(status_code=404, detail="Company test not found")
        
        if test["candidate"] != user["username"]:
            logger.error(f"Unauthorized access attempt: {user['username']} trying to access test for {test['candidate']}")
            raise HTTPException(status_code=403, detail="Not authorized to access this test")
        
        # Convert ObjectId to string for JSON serialization
        test["_id"] = str(test["_id"])
        
        logger.info(f"Successfully fetched company test details for test_id: {test_id}")
        return {"success": True, "test": test}
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error fetching company test details: {e}")
        raise HTTPException(status_code=500, detail="Failed to fetch company test details")

@app.get("/api/recruiter/company-test-results/{candidate}")
async def recruiter_get_company_test_results(candidate: str, user=Depends(recruiter_required)):
    try:
        tests = await db.assignedcompanytests.find({
            "candidate": candidate,
            "status": "Completed"
        ...
        logger.error(f"Error fetching company test results: {e}")
        raise HTTPException(status_code=500, detail="Failed to fetch company test results")
...
async def download_file(file_path: str):
    try:
        ...
        if not decoded_path.startswith(UPLOAD_DIR):
            raise HTTPException(status_code=403, detail="Access denied")
